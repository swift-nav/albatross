...
def sample_prior(cov, k):
    chol = scipy.linalg.cholesky(cov, lower=True)
    return np.dot(chol, np.random.normal(size=(cov.shape[0], k)))
...
def example_fit_and_predict(cov_func, X, y, x_star, meas_noise):
    ...
    nugget = 1e-12
    K_ss = cov_func(x_star, x_star) + nugget * np.eye(x_star.size)
